...
1. Класс AppState
   ...
   ```
## Сервисный класс ShopAPI
  Является клиентом API для взаимодействия с сервером магазина. Вот его основные характеристики:

  constructor(string, RequestInit?) Принимает базовый URL и параметры запроса (необязательно) для настройки запросов к серверу.

  Методы реализуются через интерфейс IShopApi(см. выше в Типах)
  ```
  getItems(): Promise<IItemList> Асинхронно выполняет запрос к серверу для получения списка товаров. Возвращает список товаров в формате IItemList.
  getItem(string): Promise<IItem> Асинхронно выполняет запрос к серверу для получения информации о товаре по его идентификатору. Принимает идентификатор товара в качестве параметра. Возвращает информацию о товаре в формате IItem.
  makeOrder(IOrder): Promise<IOrderResult> Асинхронно отправляет запрос на сервер для создания заказа с переданными данными заказа. Принимает данные заказа в формате IOrder. Возвращает результат создания заказа в формате IOrderResult.
  ```
  Этот класс предоставляет удобный интерфейс для работы с API магазина, предоставляя методы для получения списка товаров, информации о конкретном товаре и создания заказа. Он использует базовый класс Api для выполнения HTTP-запросов к серверу.
